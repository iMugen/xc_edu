<template>
  <div>
    <el-form :model="courseMarketForm" :rules="courseMarketFormRules" label-width="110px" ref="courseMarketForm">
      <el-form-item label="课程价格" prop="charge">
        <b v-for="charge in chargeList">
          <el-radio :label="charge.sdId" v-model="courseMarketForm.charge">{{charge.sdName}}</el-radio>
          &nbsp;&nbsp;
        </b>
        <br/>
        金额（元）：
      ...
      <el-form-item label="课程有效期" prop="expires">
        <b v-for="valid in validList">
          <el-radio :label="valid.sdId" v-model="courseMarketForm.valid">{{valid.sdName}}</el-radio>&nbsp;&nbsp;
        </b>
        <br/>
        开始时间：
        <el-date-picker :disabled="this.courseMarketForm.valid == '204002'?false:true" placeholder="选择日期" type="date"
